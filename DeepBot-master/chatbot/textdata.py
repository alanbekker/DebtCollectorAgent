...
class TextData:
        ...
        for i in range(batchSize):
            ...
            if (isinstance(sample[0],list)):
                ...
                batch.encoderSeqs.append((sample[0]))
                  # Reverse inputs (and not outputs), little trick as defined on the original seq2seq paper
    ...
    def getTestBatches(self):
        """Prepare the batches for the test data
        Return:
            list<Batch>: Get a list of the batches for the test phase
        """
        self.shuffle()

        batches = []

        def genTextNextSamples():
            """ Generator over the mini-batch training samples
            """
            for i in range(0, self.getTestSampleSize(), self.args.batchSize):
                yield self.testSamples[i:min(i + self.args.batchSize, self.getTestSampleSize())]

        for samples in genTextNextSamples():
            batch = self._createBatch(samples)
            batches.append(batch)
        return batches
    ...
    def getTestData(self):
        ...
        return self._createBatch(self.testSamples)
    ...
    def loadCorpus(self):
        ...
        if not datasetExist:  # First time we load the database: creating all files
            ...
            if not datasetExist:
                ...
                # Corpus creation
                corpusData = TextData.availableCorpus[self.args.corpus](self.corpusDir + optional,max_num_subdir=self.args.max_num_subdir)
    ...
    def loadDataset(self, filename):
        ...
        with open(dataset_path, 'rb') as handle:
            ...
            self.allSamples = data['trainingSamples']
            self.trainingSamples, self.testSamples= train_test_split(self.allSamples,test_size=self.args.test_ratio,
                                                                                random_state=42)
    ...
    def extractText(self, line,create=True):
        ...
        for i in range(len(sentencesToken)):
            ...
            tempWords = []
            for token in tokens:
                tempWords.append(self.getWordId(token,create))  # Create the vocabulary and the training sentences
