...
class Model:
    ...
    def buildNetwork(self):
        ...
        # For training only
            ...
            self.testlossFct= tf.contrib.legacy_seq2seq.sequence_loss(
                decoderOutputs,
                self.decoderTargets,
                self.decoderWeights,
                self.textData.getVocabularySize(),
            ...
            self.perplexity=tf.exp(self.lossFct) #if self.lossFct < 300 else float("inf")
            self.train_summary=tf.summary.scalar('loss', self.lossFct)  # Keep track of the cost
            self.test_summary=tf.summary.scalar('test_loss', self.testlossFct)  # Keep track of the test cost
            tf.summary.scalar('perplexity', self.perplexity)  # Keep track of the cost
    ...
    def step(self, batch):
        ...
        if not self.args.test:  # Training
            ...
            ops = (self.optOp, self.perplexity,self.lossFct)
    ...
    def test_step(self, batch):
        ...
        Args:
            batch (Batch): Input data on testing mode, input and target on output mode
        Return:
            (ops),test loss =with the associated feed dictionary
        """
