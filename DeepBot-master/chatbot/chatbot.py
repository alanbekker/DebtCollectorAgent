...
class Chatbot:
    ...
    def parseArgs(args):
        ...
        datasetArgs.add_argument('--max_num_subdir', type=int, default=1000,
                                 help='the maximal number of subdirs in ubuntu')
        ...
        trainingArgs.add_argument('--test_ratio', type=float, default=0.1, help='test_ratio rate for validation of the loss')
        ...
        try:  # If the user exit while training, we still try to save the model
            for e in range(self.args.numEpochs):
                ...
                for nextBatch in tqdm(batches, desc="Training"):
                    ...
                    _, perplexity,loss,loss_summary, summary = sess.run(ops +(self.model.train_summary,)+(mergedSummaries,), feedDict)
                    #loss,loss2,summary = sess.run([ops[2],self.model.train_summary,mergedSummaries], feedDict)
                    self.writer.add_summary(loss_summary, self.globStep)
                    self.globStep += 1
                    ...
                    if self.globStep % 100 == 0:
                        ...
                        tqdm.write("----- Step %d -- Loss %.2f " % (self.globStep, loss))

                    # Checkpoint +testData
                    if self.globStep % self.args.saveEvery == 0:
                        testBatch = self.textData.getTestBatches()
                        test_loss = 0
                        for nextBatch in tqdm(testBatch, desc="Test"):
                            ops, feedDict = self.model.test_step(nextBatch)
                            test_loss_batch, loss_test_summary = sess.run([ops ,self.model.test_summary] , feedDict)
                            #test_loss_batch, test_loss_batch2 = sess.run([ops, self.model.test_summary], feedDict)
                            test_loss += test_loss_batch
                        test_loss=test_loss / len(testBatch)
                        print ("----- Step %d -- LossTest %.2f " % (self.globStep,test_loss))
                        self.writer.add_summary(loss_test_summary, self.globStep)
                        self._saveSession(sess)
                        self.train_predictTestset()
            ...
            with open(saveName, 'w') as f:
               ...
               # for line in tqdm(lines, desc='Sentences'):
                ...
                for i in range(0,len(lines),2):
                    question=lines[i][:-1]# Remove the endl character
                    true_answer = lines[i+1][:-1]# Remove the endl character
                    #self.textData.sentence2enco(question)
                    #self.textData.getBatches()
                    if (len(question) <= self.args.maxLength and  len(true_answer) <= self.args.maxLength ):
                        self.test_batches.append([self.textData.extractText(question,create=False)[0], self.textData.extractText(true_answer,create=False)[0]])
                    #batch = self.textData._createBatch([self.textData.extractText(question,create=False)[0], self.textData.extractText(true_answer,create=False)[0]])
                    #self.test_batches.append(batch)
                    #ops, feedDict = self.model.test_step(batch)
                    #test_loss = sess.run(ops, feedDict)
                    ...
                    f.write(predString)
                batch = self.textData._createBatch(self.test_batches)
                ops, feedDict = self.model.test_step(batch)
                test_loss = sess.run(ops, feedDict)
                print('Prediction finished, {}/{} sentences ignored (too long)'.format(nbIgnored, len(lines)))


                print("The Loss in Test samples is %.2f " % (tot_test_loss))

    def train_predictTestset(self):
        ...
        with open(os.path.join(self.args.rootDir, self.TEST_IN_NAME), 'r') as f:
            ...
            batch = self.textData._createBatch(self.test_batches)
            ops, feedDict = self.model.test_step(batch)
            test_loss = self.sess.run(ops, feedDict)
            print('Prediction finished, {}/{} sentences ignored (too long)'.format(nbIgnored, len(lines)))

            print("The Loss in Test samples is %.2f " % (test_loss))
